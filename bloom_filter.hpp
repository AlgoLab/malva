...
public:
  ...
  void add_key(const string &kmer) {
    uint64_t hash = _get_hash(kmer);
    _bf[hash % _size] = 1;
  }

  void add_refkey(const string &kmer) {
    uint64_t hash = _get_hash(kmer);
    _bf[hash % _size] = 0;
  }

  bool test_key(const string &kmer) const {
    uint64_t hash = _get_hash(kmer);
  ...
  bool increment(const string &kmer) {
    if (!_mode)
      return false;
    uint64_t hash = _get_hash(kmer);
  ...
  bool increment(const string &kmer, const uint32 counter) {
    if (!_mode)
      return false;
    uint64_t hash = _get_hash(kmer);
    size_t bf_idx = hash % _size;
  ...
  uint8_t get_count(const string &kmer) const {
    if (_mode) {
      uint64_t hash = _get_hash(kmer);
      size_t bf_idx = hash % _size;
      if (_bf[bf_idx])
