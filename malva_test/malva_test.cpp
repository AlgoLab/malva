...
int equal_gt(const uint8_t size, VCFt geno, VCFt sample)
{
    ...
    {
        if (bcf_get_fmt(geno.header, geno.record, "GT")->p[1] == bcf_get_fmt(sample.header, sample.record, "GT")->p[1])
        {
            return 0;
        }
    }
...
void compare_vcf(const char *geno_vcf, const char *sample_vcf)
{
    // PRINT USED FILEs
    std::cerr << std::endl
              << "Compare \"" << geno_vcf << "\" with \"" << sample_vcf << "\"" << std::endl
              << std::endl;
     ...
     ***/
    while ((bcf_read(geno.bcf, geno.header, geno.record) == 0) && (bcf_read(sample.bcf, sample.header, sample.record) == 0))
    {
        compari++;
         ...
         ***/
        if ((geno.record->rid + 1 == sample.record->rid + 1) && (geno.record->pos + 1 == sample.record->pos + 1) && (strcmp(geno.record->d.id, sample.record->d.id) == 0) && (equal_allele(geno.record, sample.record) == 0) && (bcf_get_fmt(geno.header, geno.record, "GT")->size == bcf_get_fmt(sample.header, sample.record, "GT")->size) && (equal_gt(bcf_get_fmt(geno.header, geno.record, "GT")->size, geno, sample) == 0) && (equal_gq(geno, sample) == 0))
        {
            match++;
        }
        else
        {
            ...
            std::cerr << "<< NOT FOUND: "
                      << "#CHROM " << geno.record->rid + 1 << " #POS " << geno.record->pos + 1 << " #ID " << geno.record->d.id << " #REF " << geno.record->d.allele[0];
    ...
    std::cerr << std::endl
              << "Records Matched: " << match << ", Records Processed: " << compari << std::endl;
